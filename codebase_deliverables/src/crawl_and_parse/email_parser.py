...
data_dir = sys.argv[1]
cwd = os.getcwd()
filelist = [i for i in glob.glob(data_dir + '/*.{}'.format('csv'))]
...
if os.path.isdir(trainingd):
    ...
    os.mkdir(trainingd)
if not os.path.isdir(cwd + slash + training + slash + data_dir):
    os.mkdir(cwd + slash + training + slash + data_dir)
#does the file exist
...
for csv in filelist:
    ...
    with open(csv) as file:
        for line in file:
            ...
            outfile.write(body + '\n')
            outfile.close()
print("Worked on " + str(csv_num) + " csv files.")
fsend.write('\n')
fsubj.write('\n')
fwords.write('\n' )     
fsend.close()
